from google import google import numpy as np import unicodedata import time
...
return grams
def get_grams(string): """ From a given string, extract the unigrams and the bigrams, postprocess them and return them in a dict structure. Post processing includes incorrect grams deletion etc. - string: a string return: grams. Structure containing unigrams, bigrams and complete string. """ # First split to words words = string.split() words = [word.lower() for word in words] stopwords = frenchstopwords
...
def get_content_google(search_text): """ Google research, using google API. Get the first two pages of research, extract the description of the different items as well as the title names. """ # Single google search content = google.search(search_text, lang='en', pages=2)
...
def simple_count(choice, content): """ Simple scoring method based on preoprecessed choice occurences """ # Apply a preprocessing to the choice choice = preprocess_choice(choice)

# Compute the score by counting occurences
score = content.count(choice)
return score
def grams_count(choice, content): """ Scoring method based on n-grams. Preprocess the choice, get the unigrams, bigrams of the choice. Count then in the content and compute finale score. Final score is the sum of the occurences multiplied by a factor. score = k1 x occ_unigrams + k2 x occ_bigrams + k3 x occ_complete - choice: string, one possible choice for the answer - content: string, content to search from. """ # Initialize score score = 0
...
def answer_scores(question, choices, method='ngrams_counts'): """ Estimates score of each choice of a Multiple Choice Question (MCQ). Based on Google research, compute score for each choice based on different methods. - question: question to answer (str/unicode) - choices: list of choices (str/unicode) return index: score of each choice, all sum to 1. If nothing was found, returns list of zeros of size len(choices). """ # Force the question to unicode question = to_unicode(question)
...
return choice_scores
def answer(question, choices): """ Based on the scores of each choice, take a decision about which choice to take. - question: question to answer (str/unicode) - choices: list of choices (str/unicode) return index: index of the choosen choice """ # Get the score of each choice of the question choice_scores = answer_scores(question, choices)
